...
challenge 1: what is the index of the 64th hash
challenge 2: what is the index of the 64th hash when the hash is generated by hashing 2016 times
"""
...
def get_stretched_hash(salt: str, index) -> str:
    if index in stretched_hashes:
...
def hash_valid(salt: str, index: int, stretched=False):
    hash_function = get_hash if not stretched else get_stretched_hash
...
def get_hashes(salt: str, stretched=False):
    ...
    while len(hash_indexes) < 64:
        if hash_valid(salt, counter, stretched):
            ...
            print("hashes found: ", len(hash_indexes), end="\r")
        counter += 1
    return hash_indexes
